    ...
    thisWindow_ = {
        ...
        open: function() {
            ...
            if (view.hasOwnProperty('getViewMenu')) {
                ...
                if (getViewMenu != null) {
                    var menuItems = goog.array.filter(getViewMenu(), function(menuItem){
                        var permitted = true;
                        var requiredPermission = menuItem["permission"];
                        if (requiredPermission) {
                            var websession = codeshelf.sessionGlobals.getWebsession(); //TODO simple but effective
                            permitted = websession.getAuthz().hasPermission(menuItem["permission"]);
                        }
                        return permitted;
                    });
