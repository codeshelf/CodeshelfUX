...
export function renderContextLabel(selected) {
  if (selected.selectedFacility){
    let {description, timeZoneDisplay, production} = selected.selectedFacility;
    if (selected.selectedCustomer && selected.selectedCustomer !== 'ALL') {
      description = selected.selectedCustomer.name;
    }
    return (<span><Icon name="building" style={{marginRight: ".25em"}}/>{description}({timeZoneDisplay}) {!production && <Icon name="leaf" />}</span>);
  } else {
    return null;
  }
}
        ...
        return (<DropdownButton id="facility-dropdown" className="facility-dropdown" bsStyle="link" title={renderContextLabel(selected)}>
                <div className="sidebar-menu" style={{maxHeight: 500, overflowY: 'scroll',width: 200}}>
                {
                    availableFacilities.map((facility) => {
                        ...
                        return (
                           <ul key={domainId} style={{listStyleType: 'none', paddingLeft: 15, paddingTop: 3}}>
                              <MenuItemLink
