...
(function($) {
	...
	function DataView(options) {
		...
		// private
		var idProperty = "persistentId";  // property holding a unique row id
		...
		function compileFilter() {
			...
			var filterBody = filterInfo.body
				...
				.replace(/return true[;}]/gi, "{ _retval[_idx++] = XXXitemXXX; continue _coreloop; }")
				.replace(/return ([^;}]+?);/gi,
			             "{ if ($1) { _retval[_idx++] = XXXitemXXX; }; continue _coreloop; }");
			...
			var tpl = [
				...
				"var _retval = [], _idx = 0; ",
				"var XXXitemXXX, XXXargsXXX = _args; ",
				"_coreloop: ",
				"for (var _i = 0, _il = _items.length; _i < _il; _i++) { ",
				"XXXitemXXX = _items[_i]; ",
				"$filter$; ",
				"} ",
				"return _retval; "
				//"}"
			...
			tpl = tpl.replace(/XXXitemXXX/gi, filterInfo.params[0]);
			tpl = tpl.replace(/XXXargsXXX/gi, filterInfo.params[1]);
		...
		function compileFilterWithCaching() {
			...
			var filterBody = filterInfo.body
				.replace(/return false[;}]/gi, "{ continue _coreloop; }")
				.replace(/return true[;}]/gi, "{ _cache[_i] = true;_retval[_idx++] = XXXitemXXX; continue _coreloop; }")
				.replace(/return ([^;}]+?);/gi,
			             "{ if ((_cache[_i] = $1)) { _retval[_idx++] = XXXitemXXX; }; continue _coreloop; }");
