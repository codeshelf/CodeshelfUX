...
goog.net.WebSocket.prototype.open = function(url, opt_protocol) {
  ...
  // JBW - 19FEB14: Careful - WebSocket will compile away to a Reference Error!
  if (this.protocol_) {
    goog.log.info(this.logger_, 'Opening the WebSocket on ' + this.url_ +
        ' with protocol ' + this.protocol_);
    this.webSocket_ = new window['WebSocket'](this.url_, this.protocol_);
  } else {
    goog.log.info(this.logger_, 'Opening the WebSocket on ' + this.url_);
    this.webSocket_ = new window['WebSocket'](this.url_);
  }
